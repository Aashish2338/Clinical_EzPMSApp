...
public class AddUserActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private Spinner titleSpinner, UserTypeSpinner;
    private EditText firstName_Et, lastName_Et, Et_CountryCode, usersId_Et, Et_Password;
    ...
    private String maleFemale = "", titleId, titleName = "", emailId = "", userTypeId;
    private int isActive, userId = 0;
    private String[] titleNameArray, userType = {"Select Type", "Doctor", "Staff", "Consultant",};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Tv_Title.setText("User Registration");
    ...
    private void getUserTypeName() {
        try {
            ArrayAdapter<String> UserTypeAdapter = new ArrayAdapter<String>(getApplicationContext(), android.R.layout.simple_spinner_item, userType);
    ...
    private void getLayouUiId() {
        try {
            ...
            addUser_Btn = (Button) findViewById(R.id.addUser_Btn);
    ...
    private void checkMaleActiveData() {
        try {
            if (activeDeactive.isChecked() == true) {
                activeDeactive.setChecked(true);
                activeDeactive.setClickable(true);
                activeDeactive.setEnabled(true);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.addUser_Btn:
                if (validateUser()) {
                    if (NetworkStatus.isNetworkAvailable(mContext)) {
                        addUserProfile(titleId, firstName_Et.getText().toString().trim(), lastName_Et.getText().toString().trim(),
                                maleFemale, Et_CountryCode.getText().toString().trim(), usersId_Et.getText().toString().trim(),
    ...
    private boolean validateUser() {
        try {
            if (firstName_Et.getText().toString().isEmpty()) {
                ...
                return false;
            } else if (maleFemale.equalsIgnoreCase("")) {
                Toast.makeText(mContext, "Please select your gender!", Toast.LENGTH_SHORT).show();
                return false;
            }
        } catch (Exception exp) {
            exp.getStackTrace();
    ...
    private void addUserProfile(String titleId, String firstName, String lastName, String maleFemale, String countryCode,
        ...
        try {
            ...
            ApiClientNetwork apiClientNetwork = ApiClients.getRetrofit().create(ApiClientNetwork.class);
            Call<ClinicRegisterResponse> clinicRegisterResponseCall = apiClientNetwork.updateUserProfile(userId, firstName, lastName, maleFemale, Integer.valueOf(userTypeId),
